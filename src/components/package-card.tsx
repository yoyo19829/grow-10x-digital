  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col">
        <div
          className={`text-[10px] font-bold uppercase tracking-widest ${pkg.highlight ? "text-white/50" : "text-muted-foreground"}`}
        >
          {pkg.category}
        </div>
        <div className="mt-2 text-3xl">{pkg.emoji}</div>
        <h3
          className={`mt-3 text-2xl font-bold ${pkg.highlight ? "text-white" : "text-navy-deep"}`}
        >
          {pkg.name}
        </h3>
        <p
          className={`mt-2 min-h-[2.75rem] text-sm leading-relaxed ${pkg.highlight ? "text-white/70" : "text-muted-foreground"}`}
        >
          {pkg.tagline}
        </p>
            ...
            <div key={sec.title}>
              ...
              <ul className="space-y-2">
                {sec.items.map((it) => (
                  <li
                    key={it}
                    className={`flex items-start gap-2 text-sm ${pkg.highlight ? "text-white/85" : "text-foreground/85"}`}
                  >
        ...
        {pkg.note && (
          <div
            className={`mt-6 text-xs italic ${pkg.highlight ? "text-white/60" : "text-muted-foreground"}`}
          >
            Note: {pkg.note}
          </div>
        )}
