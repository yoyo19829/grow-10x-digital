    ...
    async function fetchPost() {
      try {
        const response = await fetch(
          `https://www.bedifly.com/wp-json/wp/v2/posts?slug=${encodeURIComponent(slug)}&_embed`,
        );
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-hero">
        ...
        <div className="pt-40 text-center text-muted-foreground">Loading article...</div>
      </main>
    );
  }
      ...
      <main className="min-h-screen bg-hero">
        ...
        <div className="min-h-screen flex items-center justify-center px-6">
          <div className="text-center">
            <h1 className="text-3xl font-bold text-navy-deep">Article not found</h1>

            <a href="/blogs" className="inline-block mt-5 text-orange font-semibold">
              ← Back to blogs
            </a>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {image && (
            <div className="max-w-5xl mx-auto mt-10 md:mt-12 overflow-hidden rounded-[1.75rem] shadow-soft">
              <img src={image} alt={imageAlt} className="block w-full h-auto object-cover" />
            </div>
          )}
